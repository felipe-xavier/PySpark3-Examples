...
spark = SparkSession.builder.appName('MaxTemp').getOrCreate()
...
pcds_df = spark.read.csv('datasets/1800.csv', schema=schema)

pcds_df.describe().show()
pcds_df.printSchema()

# Select just the rows with TMIN in var column.
min_temps_df = pcds_df[pcds_df.var == 'TMAX']
min_temps_df.show(5)
